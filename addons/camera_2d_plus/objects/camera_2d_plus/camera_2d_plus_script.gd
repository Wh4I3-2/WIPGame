...
func _ready() -> void:
	CameraManager.current = self
	
	limit_bottom += global_position.y
	limit_top += global_position.y
	limit_left += global_position.x
	limit_right += global_position.x
...
func flash(color: Color = Color.WHITE, duration: float = 0.5, hold: float = 0.0, ease = Tween.EASE_OUT, trans = Tween.TRANS_LINEAR, fade_in = false, fade_duration = 0.0) -> void:
	...
	flash_rect.color = Color(color, int(!fade_in))
	
	var tween: Tween = get_tree().create_tween() # Creating a new tween.
	tween.set_ease(ease) # Updating the tween's easing style.
	tween.set_trans(trans) # Updating the tween's transition style.
	
	## Updating the FlashRect settings.
	tween.tween_property(flash_rect, "color", color, fade_duration) # Tweening the FlashRect color.
	
	## Waiting before tweening the FlashRect back to transparent.
	await get_tree().create_timer(fade_duration + hold).timeout # Waiting before creating the tween.
		
	## Creating the tween that is going to make the camera flash.
	tween = get_tree().create_tween() # Creating a new tween.
	tween.set_ease(ease) # Updating the tween's easing style.
	tween.set_trans(trans) # Updating the tween's transition style.
	
	## Tweening the FlashRect back to transparent.
	tween.tween_property(flash_rect, "color", Color(color, 0.0), duration) # Tweening the FlashRect color.
...
var limit_target_top = 0
var limit_target_left = 0
var limit_target_right = 0
var limit_target_bottom = 0

func set_limits(_limit_top = limit_target_top, _limit_left = limit_target_left, _limit_right = limit_target_right, _limit_bottom = limit_target_bottom, duration = 0.2, ease = Tween.EASE_IN_OUT, trans = Tween.TRANS_LINEAR):
	limit_target_top = _limit_top
	limit_target_left = _limit_left
	limit_target_right = _limit_right
	limit_target_bottom = _limit_bottom
	
	var tween_top: Tween = get_tree().create_tween()
	tween_top.set_ease(ease)
	tween_top.set_trans(trans)
	tween_top.tween_property(self, "limit_top", limit_target_top, duration)
	
	var tween_left: Tween = get_tree().create_tween()
	tween_left.set_ease(ease)
	tween_left.set_trans(trans)
	tween_left.tween_property(self, "limit_left", limit_target_left, duration)
	
	var tween_right: Tween = get_tree().create_tween()
	tween_right.set_ease(ease)
	tween_right.set_trans(trans)
	tween_right.tween_property(self, "limit_right", limit_target_right, duration)
	
	var tween_bottom: Tween = get_tree().create_tween()
	tween_bottom.set_ease(ease)
	tween_bottom.set_trans(trans)
	tween_bottom.tween_property(self, "limit_bottom", limit_target_bottom, duration)
